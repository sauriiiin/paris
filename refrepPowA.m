...
%                       {'null';'nsull';'NaN';'NaN'})
    ...
    %%  GENERATE FITNESS DATA
                ...
                fprintf("Analysis for Control Hour = %0.2f Started.\n",cont_hrs);
                fprintf('Control Hour = %0.2f\n',cont_hrs)

                fpr = fetch(conn, sprintf(['select count(*) from %s ',...
                    'where hours = %0.2f and p < 0.05'],tablename_pval,cont_hrs));
                orfs = fetch(conn, sprintf(['select count(*) from %s ',...
                    'where hours = %0.2f'],tablename_pval,cont_hrs));
...
%                         210000,220000,230000,240000];
                    ...
                    for ii = 1:length(contpos)
                        temp = fetch(conn, sprintf(['select fitness from %s ',...
                            'where hours = %0.2f and pos in (%s)'],temp_fit,cont_hrs,...
